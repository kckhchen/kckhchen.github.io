...
沒錯，兩條公式壓根就長得一模一樣，只有一個地方變了——原本的機率函數是 \\(x\\) 的函數，現在成為了 \\(\theta\\) 的函數，也就是說，我不是在問給定 \\(\theta\\) 下，出現 \\(x)1, \dots,x_n\\) 的機率，而是「給定我看到的結果 \\(x_1, \dots ,x_n\\)，我的母體參數是 \\(\theta\\) 的機率為何？」我的目光焦點從實驗結果的機率轉向了母體參數的機率。
